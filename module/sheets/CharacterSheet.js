export default class CharacterSheet extends ActorSheet {
  ...
  activateListeners(html) {
    ...
    // Weapons Dropdown
  html.find('select[name="system.selectedWeapon"]').change((event) => {
  const selectedWeapon = event.target.value;
  this.onWeaponSelected(selectedWeapon);

  // Actualiza la vista de detalles del arma
  const weaponDetails = html.find('#weapon-details');
  weaponDetails.html(`
    <h3>Detalles del arma seleccionada:</h3>
    <p><strong>Daño:</strong> ${CONFIG.falloutpnp.weapons[selectedWeapon].damage}</p>
    <p><strong>Peso:</strong> ${CONFIG.falloutpnp.weapons[selectedWeapon].weight}</p>
    <p><strong>Descripción:</strong> ${CONFIG.falloutpnp.weapons[selectedWeapon].description}</p>
  `);
});

  // Método para manejar la selección de un arma
